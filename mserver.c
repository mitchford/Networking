/************************************************************************
*   Program Name: SNTPv4 Manycast Server								*
*   Description:  The server listens for any request on 224.0.0.1.		*
*	Afer receiving a packet, the server checks the date and time that	*
*	it was received. It then constructs an SNTP packet to send back to	*
*	the client. Dirctly before sending, it checks the latest date and 	*
*	time, updates the packet, then sends it. The server closes the  	*
*	socket and repeats from the beginning.								*
*																		*
...
int main( void) 
{
    ...
    while (1)
    {
       ...
       my_addr.sin_addr.s_addr = inet_addr("224.0.0.1");	/*accept all requests on 224.0.0.1 (manycast)*/
