/****************************************************************************
*   Program Name: SNTPv4 Client												*
*   Description:  The client is run with a specified host. It ensures that	*
*	a valid hostname or IP address has been entered. If it has then it		*
*	creates an SNTP client request packet which is sent to the server. The	*
...
int main( int argc, char * argv[]) 
{
    ...
    if( argc != 2)	/*ensure an argument has been entered for hostname or IP address*/
      {	
    ...
    memset( &request, 0, sizeof(request));	/*clear the request packet*/
    ...
    sec = ((reply.recieve_timestamp_1 - reply.originate_timestamp_1) + (reply.transmit_timestamp_1 - ntpsec)) / 2;	/*calculate the system clock offset*/
    usec = ((reply.recieve_timestamp_2 - reply.originate_timestamp_2) + (reply.transmit_timestamp_2 - ntpusec)) / 2;

    dsec = (ntpsec - reply.originate_timestamp_1) - (reply.transmit_timestamp_1 - reply.recieve_timestamp_1);	/*calculate the roundtrip delay*/
...
void convertTime (unsigned int *ntpsec, unsigned int *ntpusec, struct timeval time)
{
  *ntpsec = time.tv_sec + 0x83AA7E80;	/*convert the time in seconds to NTP time*/
  *ntpusec = (uint32_t)((double)(time.tv_usec+1) * (double)(1LL<<32) * 1.0e-6 );  /*convert the time in microseconds*/
}